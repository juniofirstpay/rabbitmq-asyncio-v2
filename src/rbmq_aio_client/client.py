...
class RBMQAsyncioClient:
    ...
    async def configure(self):
        ...
        async with self.__connection_pool, self.__channel_pool:
            ...
            if self.__subscriber_task:
                await asyncio.wait((self.__subscriber_task,))
            
        
            if self.__publisher_task:
                await asyncio.wait((self.__publisher_task,))
    ...
    async def create_publisher(
        ...
        while (self.__publisher_running or self.__message_queue.qsize() > 0):   
            try:
                async with self.__channel_pool.acquire() as ch: # type: ignore #type: aio_pika.abc.AbstractChannel
                    ...
                    while (self.__publisher_running or self.__message_queue.qsize() > 0):
                        try:
                            ...
                            if self.__publisher_running == False:
                                await logger.adebug("breaking while loop")
                                break
                            else:
